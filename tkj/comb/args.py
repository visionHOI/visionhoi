...
def get_args():
    ...
    parser.add_argument('--lr-text-adapter', '--lr_text_adapter', dest='lr_text_adapter', default=5e-4, type=float,
                        help='learning rate for text adapter parameters')
    ...
    # scene gate (visionhoi-main/why modification)
    parser.add_argument('--use_scene_gate', action='store_true', help='whether to use scene gate')
    parser.add_argument('--scene_gate_type', type=str, default='image', choices=['image', 'pair'], help='type of scene gate')
    parser.add_argument('--scene_gate_hidden_dim', type=int, default=128, help='hidden dimension for scene gate')
    
    # text adapter (visionhoi-main/v1 migration)
    parser.add_argument('--use-text-adapter', '--use_text_adapter', dest='use_text_adapter', action='store_true',
                        help='enable v1 text adapter (LoRA-style)')
    parser.add_argument('--text-adapter-dim', '--text_adapter_dim', dest='text_adapter_dim', type=int, default=64,
                        help='bottleneck dimension for text adapter')
    parser.add_argument('--lora-rank', '--lora_rank', dest='lora_rank', type=int, default=4,
                        help='LoRA rank used inside text adapter')
    parser.add_argument('--lora-alpha', '--lora_alpha', dest='lora_alpha', type=int, default=16,
                        help='LoRA alpha used inside text adapter')
