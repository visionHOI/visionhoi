...
# ========== 新增：导入语义适配器 ==========
from models.lora_utils2 import TextSemanticAdapter
...
class LAIN(nn.Module):
    ...
    def compute_sim_scores(self, region_props: List[dict], image, priors=None):
        ...
        for b_idx, props in enumerate(region_props):
            ...
            # Fallback: ensure global_feat is not None
            ...
            if text_features is not None:
                # Dynamic dimension adaptation (prevent mismatch between visual/text features)
                if global_feat.shape[-1] != text_features.shape[-1]:
                    if not hasattr(self, 'dim_proj'):
                        ...
                        nn.init.eye_(self.dim_proj.weight)  # Identity initialization
                ...
                try:
                    ...
                    if dist.get_rank() == 0:
                        print(
                            f"[WARNING] Similarity calculation failed: {e} | Visual dim {global_feat.shape} | Text dim {text_features_proj.shape}")
    ...
    def compute_interaction_loss(self, boxes, bh, bo, logits, prior, targets):
        ...
        # Concatenate prior scores
        prior = torch.cat(prior, dim=1).prod(0)
        x, y = torch.nonzero(prior).unbind(1)

        # Empty prior protection
        if len(x) == 0:
            return torch.tensor(0.0, device=prior.device, requires_grad=True)

        # Concatenate and filter logits
        logits = torch.cat(logits)
        logits = logits[x, y]
        prior = prior[x, y]
        labels = labels[x, y]

        # Count positive samples (distributed compatible)
    ...
    def prepare_region_proposals(self, results):
        ...
        for res in results:
            ...
            # Control number of human/object instances
    ...
    def forward(self,
        ...
        # Postprocessing and region proposal
