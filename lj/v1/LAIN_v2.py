...
# 导入适配器模块
from models.lora_utils_v2 import TextSemanticAdapter, get_adapter_parameters
...
class LAIN(nn.Module):
    ...
    # ========== 核心修改2：适配CustomCLIP的文本编码器结构 ==========
    def _setup_text_adapter(self, args):
        """挂载文本语义适配器并冻结CLIP文本编码器（适配CustomCLIP结构）"""
        # 1. 获取CustomCLIP中的文本编码器组件
        text_encoder = self.clip_head.text_encoder

        # 2. 冻结文本编码器所有参数（transformer/positional_embedding等）
        for param in text_encoder.parameters():
            param.requires_grad = False

        # 3. 初始化文本适配器并挂载到text_encoder上（避免修改CustomCLIP）
        text_dim = text_encoder.text_projection.shape[1]  # 自动获取文本特征维度（通常512）
        ...
        print(
            f"[Text Adapter] 已挂载到CLIP文本编码器 | 维度：{text_dim} → {args.text_adapter_dim} → {text_dim} | rank={args.lora_rank}")
        ...
        if self.args.use_prompt:
            if not self.training:
                if self.tp is None:
                    ...
                    # ========== 核心修改3：文本特征通过适配器优化 ==========
                    if self.args.use_text_adapter and hasattr(self.clip_head.text_encoder, 'text_adapter'):
                        text_features = self.clip_head.text_encoder.text_adapter(text_features)
